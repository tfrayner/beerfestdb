...
while (<BEERS>) {
    chop;
    my (
        $id,   $size, $brewery, $beer,   $abv, $supplier,
        undef, undef, undef,    $facing, $xs,  $ys
    ) = split(/:/);
    $size ||= 99;    # bogus
    push @casks, [ $id,
                   $size,
...
# workaround to stop Canvas bindings happening after item bindings
my $dragging_item = 0;
...
sub items {
    ...
    my $c = $TOP->Scrolled(
        qw/Canvas -width 50c -height 30c -relief sunken
    ...
    foreach ( reverse @casks ) {
        ...
        $c->createOval(
            $x, $y, $x + $diam, $y + $diam,
            -fill => 'orange',
            qw/-width 2 -tags/, "item $tag $btag",
            qw/-outline yellow -activeoutline green/
        );
        $c->createText(
            $x + $diam / 2,
            $y + $diam / 2,
            ...
            "-tags",
            "item label $tag $btag",
            qw/-anchor center -justify center -fill black/
        );
    }
    ...
    $c->CanvasBind(
        '<Any-9>' =>
            sub { create_spacer( shift, 9, $size{9} * $factor, "(space)" ) } );
    $c->CanvasBind(
        '<Any-8>' =>
            sub { create_spacer( shift, 18, $size{18} * $factor, "(space)" ) } );
    $c->CanvasBind(
        '<Any-c>' =>
            sub { create_spacer( shift, 18, $size{18} * $factor, "Cooler" ) } );
    $c->CanvasBind(
        '<Control-s>'
            => sub { save shift } );
    $c->CanvasBind(
    ...
    $c->CanvasBind(
        '<Control-a>'
            => sub { arrange_bays(shift) } );
    $c->CanvasBind(
        '<Control-p>'
            => sub { shift->postscript(qw/-file out.ps/) } );
...
sub create_spacer {
    ...
    $c->createOval(
        $x, $y, $x + $diam, $y + $diam,
        -fill => 'orange',
        qw/-width 2 -tags/, "item $tag",
        qw/-outline yellow -activeoutline green/
    );
    $c->createText(
        $x + $diam / 2,
        $y + $diam / 2,
        qw/-text/, $breweryfield, "-tags",
        "item label $tag",
        qw/-anchor center -justify center -fill black/
    );
    push @casks,
      [
        $id,   $size,    $breweryfield, undef,
...
sub items_under_area {
    ...
    my $i;
    my @casks = $c->find(
        'overlapping',    $iinfo->{areaX1}, $iinfo->{areaY1},
