...
sub _txn_preload_product_order {
    ...
    my $product_max = max(
        ...
        $order->search_related('product_id')
              ...
              ->search_related('order_batch_id', { festival_id => $festival })
              ->search_related('product_orders')
        ...
        $festival->search_related('festival_products', { product_id => $order->product_id })
