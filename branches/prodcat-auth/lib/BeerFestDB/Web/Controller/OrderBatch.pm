...
package BeerFestDB::Web::Controller::OrderBatch;
use Moose;
...
BeerFestDB::Web::Controller::OrderBatch - Catalyst Controller
...
sub index :Path :Args(0) {
    ...
    $c->response->body('Matched BeerFestDB::Web::Controller::OrderBatch in OrderBatch.');
}
...
sub list : Local {
    ...
    if ( defined $festival_id ) {
        ...
        $rs = $festival->search_related('order_batches')
    }
    else {
...
sub view : Local {
    ...
    if ( defined $id ) {
        my $batch = $c->model('DB::OrderBatch')->find($id);
        unless ( $batch ) {
        ...
        $c->flash->{error} = "Error: order_batch_id not defined.";
        $c->res->redirect( $c->uri_for('/default') );
        $c->detach();
    }
}
